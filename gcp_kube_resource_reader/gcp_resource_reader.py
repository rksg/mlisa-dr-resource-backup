...
This module reads properties of various Google Cloud resources using gcloud CLI commands
and generates Terraform variable files for both primary and disaster recovery (DR) configurations.

Supported GCP Resources:
- Compute Networks and Subnetworks
- NAT Routers and Gateways
- DataProc Clusters
- Cloud Functions
- Cloud Run Services
- Container (GKE) Clusters
- Firewall Rules
- Compute Addresses (Static IPs)
- Redis Instances
- Cloud SQL PostgreSQL Instances

Features:
- Filters resources based on VPC network and IP ranges
- Generates both primary and DR configurations
- Supports reading configuration from GCS buckets
- Supports writing generated files to GCS buckets
- Uses gcloud CLI commands to bypass SSL certificate issues

Usage:
    # Local file system
    reader = GCPResourceReader(project_id="my-project", region="us-central1")
    resources = reader.get_all_resources()
    
    # With GCS integration
    reader = GCPResourceReader(project_id="my-project", region="us-central1", use_gcs=True)
    resources = reader.get_all_resources()
"""
...
class GCPResourceReader:
    """
    GCP Resource Reader using gcloud CLI for comprehensive resource discovery.
    
    This class provides methods to fetch, filter, and process GCP resources using gcloud CLI commands,
    avoiding SSL certificate issues that commonly occur in corporate environments. It supports both
    local file system operations and Google Cloud Storage (GCS) integration.
    
    Key Features:
    - Automatic resource discovery for primary and disaster recovery (DR) sites
    - IP range management and validation for multi-site deployments
    - Gateway address calculation for subnetworks
    - Comprehensive resource filtering based on VPC networks and IP ranges
    - GCS integration for reading configuration files and writing generated outputs
    - Support for multiple GCP resource types (Compute, GKE, Cloud Functions, etc.)
    - Uses gcloud CLI instead of Python libraries for better corporate compatibility
    ...
    Architecture:
    - Uses gcloud CLI commands instead of Google Cloud Python libraries
    - Bypasses SSL certificate issues in corporate environments
    - Generates both primary and DR configurations automatically
    - Integrates with GCS for centralized configuration management
    ...
    def __init__(self, project_id: Optional[str] = None, 
        ...
        Args:
            ...
            dr_region: GCP region for DR resources
    ...
    """
    Save data to GCS using the new GCS utilities.
    
    Args:
        file_path: Path to file to store in GCS
    """

    def run_gcp_discovery(self, environment: str, cluster: str) -> List[str]:
        """
        Run GCP resource discovery for the specified environment and cluster.
        ...
        """
        try:
            print(f"🔍 Starting GCP Resource Discovery for {environment}/{cluster}")
            print("=" * 60)
            
            with open('./static-configs/config.json') as config_file:
                config_data = json.load(config_file)
            
            # Initialize reader with config data
            reader = GCPResourceReader(
                project_id=config_data[environment]['project_id'], 
                network_name=config_data[environment][cluster]['vpc'],
                region=config_data[environment]['region'],
                dr_region=config_data[environment]['dr_region'],
                ip_ranges=config_data[environment][cluster]['ip_ranges'] 
            )
            ...
            output_filename = f'./tf-vars/{environment}/{cluster}.tfvars.json'
            with open(output_filename, 'w') as f:
                json.dump(resources, f, indent=2)
            print(f"Primary resources saved to: {output_filename}")
            
            # Save DR resources to JSON file
            output_filename_dr = f'./tf-vars/{environment}/{cluster}-dr.tfvars.json'
            with open(output_filename_dr, 'w') as f:
                json.dump(resources_dr, f, indent=2)
            print(f"DR resources saved to: {output_filename_dr}")
                    
            print("✅ GCP Resource Discovery completed successfully")
            return [output_filename, output_filename_dr]
            
        except Exception as e:
            print(f"❌ GCP Resource Discovery failed: {e}")
            return None